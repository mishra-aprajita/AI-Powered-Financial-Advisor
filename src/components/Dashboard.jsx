...
const CustomPieTip = ({ active, payload }) => {
  ...
  return (
    <div className="bg-white shadow-xl rounded-xl px-4 py-2.5 border border-slate-100 text-sm">
      ...
      <p className="text-slate-500">{fmt(payload[0].value, _currency)}</p>
    </div>
  );
};
...
const CustomBarTip = ({ active, payload, label }) => {
  ...
  return (
    <div className="bg-white shadow-xl rounded-xl px-4 py-3 border border-slate-100 text-sm space-y-1">
      ...
      <p className="font-bold text-emerald-600">Income: {fmt(payload[1]?.value || 0, _currency)}</p>
      <p className="font-bold text-indigo-600">Spent: {fmt(payload[0]?.value || 0, _currency)}</p>
    </div>
  );
};

export default function Dashboard({ expenses, profile, darkMode, currency = 'INR' }) {
  ...
  return (
    <div className="fade-in space-y-5">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <StatCard icon={Wallet} label="Available Balance" value={fmt(34252, currency)} numericValue={34252}
          sub="Updated just now" color="#6366f1" trend={5.2} darkMode={darkMode} />
        <StatCard icon={TrendingUp} label="Total Spent" value={fmt(totalSpent, currency)} numericValue={totalSpent}
          sub="This month" color="#f59e0b" trend={-3.1} darkMode={darkMode} />
        <StatCard icon={Target} label="Budget Used" value={`${budgetUsed}%`}
          sub={`${fmt(totalSpent, currency)} / ${fmt(totalBudget, currency)}`} color="#10b981" darkMode={darkMode} />
              ...
              <div key={g.label}>
                <div className="flex justify-between text-sm mb-1.5 items-center">
                  ...
                  <span className="font-bold text-xs" style={{ color: g.color }}>
                    {fmt(g.current, currency)} <span style={{ color: textMuted }} className="font-normal">/ {fmt(g.goal, currency)}</span>
                  </span>
                </div>
          ...
          <div className="mt-3 space-y-2">
            {top5.map(item => (
              <div key={item.name} className="flex items-center justify-between text-xs">
                ...
                <span className="font-bold" style={{ color: textMain }}>{fmt(item.value, currency)}</span>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="card p-6 fade-in lg:col-span-3" style={{ background: card, borderColor: border }}>
          ...
          <ResponsiveContainer width="100%" height={245}>
            <BarChart data={MONTHLY_DATA} barSize={18} barGap={4}>
              ...
              <YAxis tick={{ fontSize: 11, fill: textMuted }} axisLine={false} tickLine={false} tickFormatter={v => v >= 100000 ? `${getSymbol(currency)}${(v/100000).toFixed(1)}L` : `${getSymbol(currency)}${(v/1000).toFixed(0)}k`} />
            ...
            return (
              <div key={item.name} className="flex flex-col gap-2">
                ...
                <div className="flex justify-between text-xs">
                  ...
                  <span style={{ color: textMuted }}>{fmt(budget, currency)}</span>
                </div>
              </div>
            );
          })}
