...
const INSIGHTS = [
  { icon: '🔥', title: 'Highest spend day', value: 'Saturday', sub: '₹4,200 avg', color: '#f97316' },
  { icon: '💤', title: 'Lowest spend day', value: 'Monday/Thursday', sub: '₹0 avg', color: '#10b981' },
...
export default function Analytics({ expenses, budgets, darkMode }) {
      ...
      <div className="bg-white shadow-xl rounded-xl px-4 py-3 border border-slate-100 text-sm">
        ...
        {payload.map(p => (
          <p key={p.dataKey} className="font-bold" style={{ color: p.color }}>
            {p.dataKey === 'income' ? 'Income' : 'Spent'}: ₹${p.value?.toLocaleString('en-IN')}
          </p>
        ))}
      </div>
    );
  ...
  return (
    <div className="fade-in space-y-6">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {[
          { label: 'Net Savings', value: `₹${(80000 - totalSpent).toLocaleString('en-IN')}`, icon: '💰', color: '#10b981', positive: true },
          { label: 'Savings Rate', value: `${savingsRate}%`, icon: '📈', color: '#6366f1', positive: true },
          { label: 'Avg Daily Spend', value: `₹${(totalSpent / 22).toFixed(0)}`, icon: '📅', color: '#f59e0b', positive: null },
          { label: 'Budget Variance', value: `₹${(totalBudget - totalSpent).toLocaleString('en-IN')}`, icon: '🎯', color: '#a855f7', positive: totalBudget > totalSpent },
        ].map(k => (
              ...
              <div className="flex-1 min-w-0">
                <div className="flex justify-between text-sm mb-1.5">
                  ...
                  <div className="flex items-center gap-3 text-xs">
                    <span className="font-bold" style={{ color: textMain }}>₹${row.spent.toLocaleString('en-IN')}</span>
                    <span style={{ color: textMuted }}>/ ₹${row.budget.toLocaleString('en-IN')}</span>
                    <span className={`font-bold px-2 py-0.5 rounded-full text-xs ${row.diff >= 0 ? 'text-emerald-600 bg-emerald-50' : 'text-red-500 bg-red-50'}`}>
                      {row.diff >= 0 ? `₹${row.diff} left` : `₹${Math.abs(row.diff)} over`}
                    </span>
                  </div>
                </div>
